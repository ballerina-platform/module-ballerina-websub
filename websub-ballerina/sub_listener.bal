...
public class Listener {
    ...
    # + return - The `websub:Listener` or an `error` if the initialization failed
    ...
    public isolated function attach(SubscriberService subscriberService, string[]|string? name = ()) returns error? {
        ...
        if configuration is SubscriberServiceConfiguration {
            ...
            RequestHandler handler = check new (subscriberService);
            string[] methodNames = handler.getServiceMethodNames();
            self.httpService = check new(handler, configuration?.secret, methodNames);
